...
import { View, Text, Image, ScrollView, StyleSheet, Animated, StatusBar } from 'react-native';

const HEIGHT_HEADER = 80 + 10;
...
export default class AnimFive extends React.PureComponent {
  ...
  render() {
    const scaleBackgroundInterpolate = this.scrollYValue.interpolate({
    ...
    const translateYBackgroundInterpolate = this.scrollYValue.interpolate({
    ...
    const scaleTitleInterpolate = this.scrollYValue.interpolate({
      inputRange: [0, HEIGHT_HERO],
      outputRange: [2, 1],
      extrapolate: 'clamp',
    });

    const translateYTitleInterpolate = this.scrollYValue.interpolate({
    ...
    const opacityOverlayInterpolate = this.scrollYValue.interpolate({
    ...
    return (
      <View style={styles.screen}>
        ...
        <Animated.View style={[styles.background, {
          ...
          <Image style={styles.image} resizeMode="cover" source={{ uri: 'http://lorempixel.com/output/nature-q-c-960-400-3.jpg' }} />
          <Animated.View  style={[styles.overlay, { opacity: opacityOverlayInterpolate }]} />
        </Animated.View>
